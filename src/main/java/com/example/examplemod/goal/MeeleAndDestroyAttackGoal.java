...
public class MeeleAndDestroyAttackGoal extends Goal {
    ...
    public MeeleAndDestroyAttackGoal(PathfinderMob p_25552_, double p_25553_, boolean p_25554_,int p_25555) {
        this.mob = p_25552_;
    ...
    private void checkTarget(LivingEntity target) {
        Path path1 = this.mob.getNavigation().createPath(target, 0);
        ...
        double dstToTargetWithoutY = mob.distanceToSqr(target.getX(), mob.getY(), target.getZ());
        if (dstSqrt > 36.0D || (canReachToTarget && dstToTargetWithoutY > 4.0D)) {
            mob.level().destroyBlockProgress(mob.getId(), blockToBreak, -1);
            resetBlockToBreak();
        }
        if (blockToBreak != null) {
    ...
    private boolean canReachToTarget(LivingEntity target) {
        double distanceToTargetWithoutY = this.mob.distanceToSqr(target.getX(), this.mob.getY(), target.getZ());
        boolean canReachToTarget = unReachTickTimer < 40 && distanceToTargetWithoutY > 4.0D;
    ...
    private Path getPathZ(LivingEntity target) {
        blockIsBroken = false;
        Path pathz = this.mob.getNavigation().createPath(target, 0);
